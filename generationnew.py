...
class Lookahead:
    ...
    def _tensor_or_none(token, device):
        if token is None:
                return None  # Explicitly return None if the token is None

        if isinstance(token, torch.Tensor):
                return token.to(device)  # Move tensor to the specified device
        return torch.tensor(token, device=device, dtype=torch.long)  # Create tensor on the device

        # Assuming input_ids is defined earlier in the code
        device = input_ids.device  # Derive the device from input_ids

        # Prepare the tensors for generation configuration
        bos_token_tensor = _tensor_or_none(
            getattr(self.model.config, "bos_token_id", None), device=device
        )
        eos_token_tensor = _tensor_or_none(
            getattr(self.model.config, "eos_token_id", None), device=device
        )
        pad_token_tensor = _tensor_or_none(
            getattr(self.model.config, "pad_token_id", None), device=device
        )
        decoder_start_token_tensor = _tensor_or_none(
            getattr(self.model.config, "decoder_start_token_id", None), device=device
        )

        # Assign the tensors to the generation configuration
        ...
        #self._prepare_special_tokens_for_generation(generation_config=generation_config, device = input_ids.device)

        '''# Then call the method correctly
        stopping_criteria = self.model._get_stopping_criteria(
            generation_config = GenerationConfig,  # This satisfies the expected parameter
            stopping_criteria=stopping_criteria,  # Optional, pass existing stoppinriteria  # Optional, pass tokenizer if stop strings are involved
        )'''

                # prepare stopping criteria
        self.stopping_criteria = self.model._get_stopping_criteria(
            max_length=max_length, max_time=max_time, stopping_criteria=stopping_criteria,generation_config = self.model.generation_config
        )
...
class Generator(GenerationMixin):
    ...
    def generate(
        ...
        exponential_decay_length_penalty: Optional[Tuple[Union[int, float]]] = None,
        **model_kwargs,
    ) -> Union[GreedySearchDecoderOnlyOutput, SampleOutput, BeamSearchOutput, BeamSampleOutput, torch.LongTensor]:
        r"""
...
def _tensor_or_none(token, device):
        ...
        if eos_token_id is None and hasattr(self.model.config, "decoder"):
            if decoder_start_token_id is not None and hasattr(decoder_start_token_id, "ndim") and decoder_start_token_id.ndim == 1:
        ...
        if self.model.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            # Use the generation configuration from the model
            generation_config = self.model.generation_config

            # Update model_kwargs for encoder-decoder models
        ...
        if self.model.config.is_encoder_decoder:
            input_ids, *other_outputs = self.model._prepare_decoder_input_ids_for_generation(
                batch_size,
                decoder_start_token_id=torch.tensor(
                    generation_config.decoder_start_token_id 
                    if generation_config.decoder_start_token_id is not None 
                    else self.generation_config.decoder_start_token_id,
                    device=inputs_tensor.device  # Ensure the tensor is on the correct device
                ),
                model_kwargs=model_kwargs,
                model_input_name=model_input_name,
            ...
            # Now, you can use the shape of input_ids tensor
        ...
        if is_group_beam_gen_mode and do_sample is True:
          ...
          # Step 7: Prepare distribution pre-processing samplers
        logits_processor = self.model._get_logits_processor(
            generation_config=generation_config,  # Pass the complete GenerationConfig object
            repetition_penalty=generation_config.repetition_penalty,
            no_repeat_ngram_size=generation_config.no_repeat_ngram_size,
            encoder_no_repeat_ngram_size=generation_config.encoder_no_repeat_ngram_size,
            input_ids_seq_length=input_ids_seq_length,  # Use the appropriate variable for sequence length
            encoder_input_ids=inputs_tensor,  # This may not be in generation_config; pass explicitly
            bad_words_ids=generation_config.bad_words_ids if hasattr(generation_config, 'bad_words_ids') else None,
            ...
            remove_invalid_values=generation_config.remove_invalid_values,
            exponential_decay_length_penalty=generation_config.exponential_decay_length_penalty,
            logits_processor=logits_processor,  # Assuming you're chaining processors
        ...
        '''logits_processor = LogitsProcessorList([
            ExponentialDecayLengthPenalty(exponential_decay_length_penalty=generation_config.exponential_decay_length_penalty)
            # Other logits processors can be added here
        ])'''

        '''        logits_processor = self.model._get_logits_processor(
            generation_config=generation_config,  # Pass the entire config
            input_ids_seq_length=input_ids_seq_length,
            encoder_input_ids=inputs_tensor,
            prefix_allowed_tokens_fn=prefix_allowed_tokens_fn,
            logits_processor=logits_processor,  # Pass existing processors
        )
'''
        ...
        # Now, call this method before using `_get_stopping_criteria`
# Assuming you have generation_config initialized
        self._prepare_special_tokens_for_generation(generation_config=generation_config, device = input_ids.device)

                        # Then call the method correctly
        stopping_criteria = self.model._get_stopping_criteria(
        generation_config=generation_config,  # This satisfies the expected parameter
        stopping_criteria=stopping_criteria,  # Optional, pass existing stoppinriteria  # Optional, pass tokenizer if stop strings are involved
        )
